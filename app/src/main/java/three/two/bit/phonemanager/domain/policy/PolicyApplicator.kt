 ...
 */
@Singleton
class PolicyApplicator @Inject constructor(private val preferencesRepository: PreferencesRepository) {
    companion object {
     ...
     */
    private suspend fun applySetting(key: String, value: Any): Boolean = when (key) {
        KEY_TRACKING_ENABLED -> {
     ...
     */
    fun isSettingLocked(policy: DevicePolicy?, settingKey: String): Boolean = policy?.isLocked(settingKey) ?: false
     ...
     */
    fun getPolicyValue(policy: DevicePolicy?, settingKey: String): Any? = policy?.getPolicyValue(settingKey)

    /**
     * Get a human-readable name for a setting key.
     */
    fun getSettingDisplayName(settingKey: String): String = when (settingKey) {
 ...
 */
private fun Any.toFloat(): Float = when (this) {
    is Float -> this
 ...
 */
data class PolicyFailure(val settingKey: String, val error: String)
